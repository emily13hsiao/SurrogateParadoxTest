...
```{r}
test_assumptions(s0 = s_c, y0 = y_c, s1 = s_t, y1 = y_t, type = "sd", all_results = FALSE)
...
```
Truth be told, these won't make sense without seeing the manuscript. But essentially, the null hypothesis of the test is that the stochastic dominance assumption does hold. The alternative is that it does not hold i.e., there exists some s where dominance does not hold. This gives the value of our test statistic **s_hat**, the p-value from testing this null hypothesis **p.value**, and whether we reject the null hypothesis **reject**. 

Now let's test monotonicity; this does involve resampling, so set the seed so your results are reproducible. This function used to be computationally inefficient but we now depend on and use the **MonotonicityTest** package which is much faster. Below, we set the number of bootstrap samples to 200 i.e., **monotonicity_bootstrap_n=200**. 
...
These results show that the test indicates that the monotonicity assumption holds in both groups. 
...
p <- ggplot() + 
  ...
  scale_colour_identity("Group", guide = "legend", 
...
To use this function, the parameters you need are the S and Y data from the treatment and control group in Study A (**s0.A, y0.A, s1.A, y1.A**), the S values of the treatment and control group in Study B (**s0.B, s1.B**), the variance vector which composes the diagonal elements of $Sigma$ (var_vec), and a length-3 vector $B$ (period). The elements of the period vector represent proportions of the width of the support of $S$ values in Study A. Let's take a look at an example.
...
To use this function, the parameters you need are the S and Y data from the treatment and control group in Study A (**s0.A, y0.A, s1.A, y1.A**), the S values of the treatment and control group in Study B (**s0.B, s1.B**), and the variance vector which composes the diagonal elements of $Sigma$ (**var_vec**).
...
# Resilience Probability: Meta-Analytic Setting

The function **meta_analytic_resilience** can calculate the resilience probability for the special setting in which you have many prior studies i.e., many "Study A" studies. This function uses a Gaussian Process class, as described above, but is able to estimate the variance parameters based on the prior studies. Therefore, the user does not need to specify the variance parameters. This method is described in Hsiao E and Parast L, "A Functional-Class Meta-Analytic Framework for Quantifying Surrogate Resilience" (Under Review).

Let's take a look at the example. The dataset **dataA** contains hypothetical data from 25 prior studies. Each dataset includes measurements on the surrogate (S), primary outcome (Y), and treatment group (G). The format of this dataset is exactly the format that the function will require. 
...
The dataset **dataB** contains hypothetical data from a new trial where only surrogate values in the control group (s0.B) and surrogate values in the treated group were measured (s1.B). 
...
The **meta_analytic_resilience** function calculates the resilience probability for the new study (**dataB**) using the information learned from the prior studies (**dataA**) and assuming a Gaussian Process function class. The function uses random data generation and thus, we set a seed to ensure reproducibility. The primary arguments, besides the data, are the degree for the mean function (**degree**), whether a spline should be used for the mean function (**use_spline**), whether the standard error should be estimated (**calculate_se**) and if so, whether the analytic standard error should be calculated (**try_analytic**). Below, we apply this function first with degree = 1 (linear), no splines, and no standard error.  
...
Next, we apply the function with degree = 1 (linear), with splines, and with standard error estimation using the analytic estimator.

```{r}
set.seed(1)
result = meta_analytic_resilience(data=dataA, s0.B = dataB$s0.B, s1.B = dataB$s1.B, 
degree = 1, use_spline = TRUE, calculate_se = TRUE, try_analytic = TRUE)
result
```

Lastly, we apply the function with degree = 3 (cubic), with splines, and with standard error estimation using the nonparametric boostrap.

```{r}
#apply function, uncertainty quantification, bootstrap SE
set.seed(1)
result = meta_analytic_resilience(data=dataA, s0.B = dataB$s0.B, s1.B = dataB$s1.B, 
degree = 3, use_spline = TRUE, calculate_se = TRUE, try_analytic = FALSE)
result
```

Regardless of the estimation approach, these results show a large probability of the surrogate paradox in the new study. 
