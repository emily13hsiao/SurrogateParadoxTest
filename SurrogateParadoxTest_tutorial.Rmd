...
Now let's test montonicity; this does involve resampling, so set the seed so your results are reproducible. This function takes a long time and to speed up this tutorial, I have set **monotonicity_bootstrap_n = 10**; please do not use 10 with real data, use 200 or more i.e. **monotonicity_bootstrap_n=200**. We are working on making this faster.

```{r}
set.seed(12)
test_assumptions(s0 = s_c, y0 = y_t, s1 = s_t, y1 = y_t, type = "monotonicity", all_results = FALSE, monotonicity_bootstrap_n = 10)
```

These results show that the test indicates that the montonicity assumption holds in both the control and treated group. 
...
This indicates that the non-negative residual treatment effect assumption holds here. 

To test them all, and get all the results, use **type = "all"** or leave unspecified since "all" is the default:

```{r}
set.seed(1)
test_assumptions(s0 = s_c, y0 = y_t, s1 = s_t, y1 = y_t, type = "all", all_results = FALSE, monotonicity_bootstrap_n = 10)
```
